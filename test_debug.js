...
`);
    ...
    AltText = _AltText;
    _TouchManager = class _TouchManager {
      constructor({
        container,
    ...
    onTouchStart_fn = function(evt) {
      var _a, _b, _c;
      if (((_a = __privateGet(this, _isPinchingDisabled)) == null ? void 0 : _a.call(this)) || evt.touches.length < 2) {
        return;
      }
    ...
    processItems_fn = function(items) {
      ...
      __privateGet(this, _layoutTextParams).ctx ??= __privateMethod(_a = _TextLayer, _TextLayer_static, getCtx_fn).call(_a, __privateGet(this, _lang));
    ...
    appendText_fn = function(geom) {
      ...
      const fontAscent = fontHeight * __privateMethod(_a = _TextLayer, _TextLayer_static, getAscent_fn).call(_a, fontFamily, __privateGet(this, _lang));
      let left, top;
      if (angle === 0) {
        left = tx[4];
        top = tx[5] - fontAscent;
    ...
    _DrawingEditor = class _DrawingEditor extends AnnotationEditor {
      constructor(params) {
        super(params);
    ...
    _InkDrawOutline = class _InkDrawOutline extends Outline {
      constructor() {
        super(...arguments);
    ...
    InkDrawOutline = _InkDrawOutline;
    _InkDrawingOptions = class _InkDrawingOptions extends DrawingOptions {
    ...
    InkDrawingOptions = _InkDrawingOptions;
    _InkEditor = class _InkEditor extends DrawingEditor {
      constructor(params) {
        super({
          ...params,
          name: "inkEditor"
    ...
    __webpack_exports__TouchManager = __webpack_exports__.TouchManager;
    ...
    __webpack_exports__stopEvent = __webpack_exports__.stopEvent;
...
__export(main_exports, {
  default: () => PdfAnnotatorPlugin
});
module.exports = __toCommonJS(main_exports);
var import_obsidian5 = require("obsidian");

// src/types.ts
var VIEW_TYPE_PDF_ANNOTATOR = "pdf-annotator-view";
...
// src/PdfAnnotatorView.ts
var import_obsidian4 = require("obsidian");

// src/PdfRenderer.ts
var pdfjsLib = null;
var TextLayerClass = null;
async function getPdfjs() {
  if (!pdfjsLib) {
    pdfjsLib = await Promise.resolve().then(() => (init_pdf(), pdf_exports));
...
var PdfRenderer = class {
  constructor() {
    this.pdfDoc = null;
    this.renderedPages = /* @__PURE__ */ new Map();
    this.scale = 1.5;
  }
...
var AnnotationStore = class {
  ...
  getAnnotationPath(pdfFile) {
    return pdfFile.path + ".annotations.json";
  }
  async loadAnnotations(pdfFile) {
    this.file = pdfFile;
    this.annotations = [];
  ...
  updateAnnotation(id, updates) {
    ...
    if (idx !== -1) {
      this.annotations[idx] = { ...this.annotations[idx], ...updates, modified: (/* @__PURE__ */ new Date()).toISOString() };
    }
  }
  getAnnotation(id) {
...
var HighlightManager = class {
  constructor(renderer, store, annotationLayer, app) {
  ...
  setup(scrollContainer, file) {
    this.scrollContainer = scrollContainer;
    this.file = file;
    this.abortController = new AbortController();
    const signal = this.abortController.signal;
    scrollContainer.addEventListener("mouseup", (e) => {
      if (e.target.closest(".pdf-annotator-highlight")) return;
      setTimeout(() => this.handleTextSelection(e), 10);
    }, { signal });
    scrollContainer.addEventListener("click", (e) => {
      const highlightEl = e.target.closest(".pdf-annotator-highlight");
      if (highlightEl) {
  ...
  handleTextSelection(e) {
    ...
    const range = selection.getRangeAt(0);
    const pageContainer = range.startContainer.closest?.(".pdf-annotator-page") ?? range.startContainer.parentElement?.closest(".pdf-annotator-page");
  ...
  createHighlight(selection, pageNumber, color) {
    const range = selection.getRangeAt(0);
  ...
  buildSelectors(selection, range, renderedPage) {
  ...
  calculateTextOffsets(range, textLayerDiv) {
  ...
  mergeRects(rects) {
    ...
    const filtered = rects.filter((r) => r.x2 - r.x1 > 1e-3 && r.y2 - r.y1 > 1e-3);
    if (filtered.length === 0) return rects.slice(0, 1);
    const merged = [];
    let current = { ...filtered[0] };
    for (let i = 1; i < filtered.length; i++) {
      const r = filtered[i];
      if (Math.abs(r.y1 - current.y1) < 5e-3 && Math.abs(r.y2 - current.y2) < 5e-3) {
  ...
  showAnnotationMenu(e, annotationId) {
    const annotation = this.store.getAnnotation(annotationId);
    if (!annotation) return;
    const menu = new import_obsidian3.Menu();
    menu.addItem((item) => {
      item.setTitle("Add Note").setIcon("edit").onClick(() => this.editNote(annotationId));
    });
    for (const [name, hex] of Object.entries(HIGHLIGHT_COLORS)) {
      menu.addItem((item) => {
        item.setTitle(`Color: ${name}`).onClick(() => {
  ...
  editNote(annotationId) {
    ...
    new NoteModal(this.app, annotation.comment, (newComment) => {
      ...
      this.annotationLayer.updateHighlightComment(annotationId, !!newComment);
      this.store.saveAnnotations();
    }).open();
  }
  destroy() {
...
// src/AnnotationLayer.ts
var AnnotationLayer2 = class {
  constructor() {
    this.highlightElements = /* @__PURE__ */ new Map();
  }
  renderHighlight(renderedPage, annotation) {
    const pageSelector = annotation.selectors.find(
      (s) => s.type === "PageSelector"
    );
    if (!pageSelector) return;
    const { viewport, annotationLayerDiv } = renderedPage;
    const elements = [];
    for (const rect of pageSelector.rects) {
      ...
      if (annotation.comment) {
        highlightEl.classList.add("has-comment");
      }
  ...
  updateHighlightComment(annotationId, hasComment) {
    const elements = this.highlightElements.get(annotationId);
    if (elements) {
      elements.forEach((el) => {
...
// src/PdfAnnotatorView.ts
var PdfAnnotatorView = class extends import_obsidian4.FileView {
  ...
  async onLoadFile(file) {
    ...
    try {
      ...
      for (let i = 1; i <= numPages; i++) {
        ...
        this.scrollContainer.appendChild(rendered.container);
...
// src/main.ts
var PdfAnnotatorPlugin = class extends import_obsidian5.Plugin {
  async onload() {
    this.registerView(
